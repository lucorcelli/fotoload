...
app.use('/imagens', express_1.default.static(path_1.default.join(__dirname, 'public/imagens')));
...
app.get('/produtos/notas', async (req, res) => {
  try {
    ...
    notas.forEach((n) => {
      ...
      if (!agrupado[id]) agrupado[id] = { soma: 0, total: 0 };
      if (n.nota != null) {
...
app.post('/produtos', async (req, res) => {
  try {
    const { nome, descricao, referencia_fabrica, codigo_barras, usuario_id, categoria_id } =
      req.body;
